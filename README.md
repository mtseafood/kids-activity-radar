...
輸出為 `activities.json`。預設涵蓋中部四縣市：**台中、苗栗、彰化、南投**。
...
| 文化部全國藝文活動（culture_cloud） | 全國開放資料 API（cloud.culture.tw），**一個來源涵蓋全台各縣市**，依地點自動判斷縣市 |
...
python scraper.py --cities 台中市 苗栗縣 彰化縣 南投縣   # 中部四縣市（每日自動更新採用）
...
sources/
  ...
  kkday.py
  epochtimes.py
  taichung_culture.py     # 台中市文化局開放資料
  culture_cloud.py        # 文化部全國藝文活動（涵蓋全台各縣市）
```
